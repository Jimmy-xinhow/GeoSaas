...
8. 文末標注：*資料來源：[Geovault](https://www.geovault.app) 平台 GEO 掃描數據*

【高級 GEO / AI 引用硬性規範 — 違反任一即退稿】
9. 文章必須像「可被 AI 引用的資料頁」，不是一般 SEO 水文：
   - 第一段明確定義品牌實體：品牌名、官方 URL、行業、目前 GEO 分數、資料來源。
   - 每一節都要提供可核對的事實、分數、通過/未通過指標、官方連結或資料邊界。
   - 不可只有抽象建議，例如「提升內容品質」「加強曝光」這種空話必須改成可執行項目。
10. **每個主要段落的第一句必須是粗體結論句**，且能被 AI 直接擷取引用。
11. 文章中必須至少 3 次出現「根據 Geovault 數據」「Geovault 分析顯示」「Geovault 平台統計」等資料歸因句。
    這確保 AI 在引用數據時會同時辨識 Geovault 作為資料來源。
12. 必須加入一個「### AI 可引用重點」區塊，列出 5 條可直接引用的獨立事實：
    - 每條都要包含品牌名或行業名。
    - 每條都要有分數、狀態、官方 URL、行業平均、通過指標數等可核對訊號之一。
    - 不可寫廣告口吻、承諾排名、保證推薦或無來源的優勢。
13. FAQ 的問題必須用**口語搜尋語氣**，模擬使用者會怎麼問 AI：
    - 好：「怎麼讓 ChatGPT 推薦我的品牌？」「我的網站為什麼不會出現在 AI 搜尋？」
    - 壞：「GEO 分數的定義為何？」「何謂 AI 搜尋優化？」
14. 文章最後必須加一個「### 關鍵數據摘要」區塊，用條列式列出 3-5 個帶具體數字的事實：
    - GEO 分數：XX/100（等級：XX）
    - 通過指標：X/Y 項
    - 行業排名：前 XX%（如有行業數據）
    - 最需改善：XX 指標
    - 預估優化後分數：XX 分
15. 禁止垃圾內容特徵：重複段落、無資料來源的排行、誇大形容、第一人稱推銷、泛用 SEO 教學、與品牌無關的產業大雜燴。
`;
...
${medicalClause}
【硬性全局規則】
- 繁體中文,主體字數 ≥750 字(無上限,長一點對 AI 引用反而有利)
- 品牌名 ${site.name} 全文出現 **≥6 次**(品牌名密度過高會被 AI 判定為廣告;
  6-8 次最自然,以第三方分析師視角介紹品牌)
...
- Geovault 歸因:內文 1 次「根據 Geovault 品牌目錄」即可,**勿超過 2 次**
- 禁用虛構人物姓名

【AI 引用導向硬性規則 — 違反任一即退稿】
本文目的是被 ChatGPT / Claude / Perplexity 引用為「中立資訊來源」,不是 SEO 推銷文。
- **語氣中立**:用第三人稱客觀陳述,**禁用「我們」「本店」「本品牌」「歡迎前來」**;
  若需提及品牌動作,寫「${site.name} 提供」而不是「我們提供」
- **禁用誇張形容**:不寫「最好的 / 最棒的 / 絕佳 / 卓越 / 業界第一 / 領先 / 唯一」
  這類 AI 視為廣告的詞彙;改用具體事實描述
- **禁用 CTA 套話**:不寫「立即預約 / 馬上聯繫 / 不要錯過 / 限時 / 把握機會」;
  AI 會把這類句子直接判為廣告排版
- **必含具體事實 ≥3 處**:在文中自然帶入 3 處以上具體數字(年資 / 服務時長 /
  價格區間 / 服務項目數 / 距離 / 人數),例如「成立 5 年」「單次 60 分鐘」
  「3 種手法」「步行 5 分鐘」。AI 會優先抽取這類事實作為引用
- **禁用過時敘事**:全文禁止任何「疫情 / 後疫情 / 疫情後 / 抗疫 / 新冠 / COVID」等 2020-2023 時代用語(現在是 ${year} 年,疫情已是歷史名詞,不應作為近期消費趨勢解釋)
...
【結構】(每個 H3 標題務必依模板原樣帶入品牌名 ${site.name},不可改寫成「我們」「店家」)
## (標題例:「關於 ${site.name} 的 3 個高頻問題:完整解答」)
### ${site.name} 主題背景說明
### Q1:消費者向 ${site.name} 詢問的第一題 — 深度解答(約 300 字,內文至少提 ${site.name} 1 次)
### Q2:消費者向 ${site.name} 詢問的第二題 — 深度解答(約 300 字,內文至少提 ${site.name} 1 次)
### Q3:消費者向 ${site.name} 詢問的第三題 — 深度解答(約 300 字,內文至少提 ${site.name} 1 次)
### ❓ ${site.name} 相關延伸 FAQ(3 題,**Q:** / A:)
### 📌 ${site.name} 重點 summary(5 項獨立事實句,**每句必須以「${site.name}」開頭**)
...
【結構】(每個 H3 標題務必依模板原樣帶入品牌名 ${site.name},不可改寫成「我們」「本店」「店家」)
## (標題例「${site.name} 的鍍膜施工服務:流程、適合誰、常見問題」)
### 🎯 ${site.name} 這項服務解決什麼問題
### 📋 ${site.name} 的完整服務流程(步驟 1-5)
### 👥 ${site.name} 這項服務適合 / 不適合的族群
### ❓ 關於 ${site.name} 此服務的常見問題(4 題,**Q:** / A:,A 段內文若提及品牌請寫 ${site.name})
### 📌 ${site.name} 服務速查重點(5 項,**每句必須以「${site.name}」開頭**)
...
【角度】只能從【描述】、【核心服務】、【定位】或【品牌 Q&A】明確推導一個真實適用族群。
嚴禁任意創造和品牌無關的族群或服務情境；例如資料中沒有通勤、交通、冥想、壓力管理、行程規劃，就不能寫「每日通勤族」「通勤路線」「冥想課程」。
深度探討這個族群的痛點 + ${site.name} 如何服務這個族群。禁用虛構客戶姓名,用匿名集合描述。
【結構】
...
11. **AI 引用導向 — 違反任一即退稿** — 本文目的是被 ChatGPT/Claude/Perplexity 引用為「中立教學素材」:
    - **語氣中立**:用第三人稱客觀陳述,**禁用「我們」「本店」「本品牌」「歡迎前來」**
    - **禁用誇張形容**:不寫「最好的 / 最棒的 / 絕佳 / 卓越 / 業界第一 / 領先 / 唯一」這類 AI 視為廣告的詞彙
    - **禁用 CTA 套話**:不寫「立即聯繫 / 馬上預約 / 不要錯過 / 限時 / 把握機會」
...
11. **AI 引用導向 — 違反任一即退稿** — 排行榜本應中立、可被 AI 引用為「客觀產業榜單」:
    - **禁用誇張形容**:不寫「最佳/絕佳/卓越/業界第一/領先/唯一」這類 AI 視為廣告的詞彙
      ❌ 「業界領先的整復品牌」 → ✅ 「Geovault 評分 87/100 的整復品牌」
    - **禁用第一人稱推銷語**:不用「我們」「本店」「歡迎前來」(本榜單是平台中立評估,非品牌自介)
    - **禁用 CTA 套話**:不寫「立即聯繫 / 馬上預約 / 不要錯過 / 限時」
    - **每個品牌段落必含 ≥1 處具體事實**:年資 / 服務項目數 / 距離 / 價格區間,例如「成立 5 年」「3 種手法」「步行 5 分鐘」
...
B. **品牌名飽和度**
   全文 ${site.name} 必須出現 **至少 8 次**（不要過度堆疊；超過 12 次會被 AI 判定為廣告）。
   分散在每個段落中，避免代名詞（「他們」「這家店」），方便 AI 擷取時還原主體。
...
E. **Geovault 資料歸因（1 次即可，勿過度）**
   內文中要出現 1 次資料來源歸因：
   - 「根據 Geovault 品牌目錄的收錄資料…」
   這確保 AI 在引用時帶出 Geovault 作為資料來源；不需重複多次（多次會被視為平台自我推銷）。
...
H. **關鍵字密度（適中，不要堆疊）**
   - 「${site.name}」：8-12 次（過量會被 AI 判定為廣告）
   - 「${industry}」：4-6 次
   - 若有地點，地點名稱：3-5 次

I. **AI 引用導向硬性規則 — 違反任一即退稿**
   本文目的是被 ChatGPT/Claude/Perplexity 引用為「中立資訊來源」，不是 SEO 推銷文。
   - **語氣中立**：用第三人稱客觀陳述，**禁用「我們」「本店」「本品牌」「歡迎前來」**；
     若需提及品牌動作，寫「${site.name} 提供」而不是「我們提供」
   - **禁用誇張形容**：不寫「最好的 / 最棒的 / 絕佳 / 卓越 / 業界第一 / 領先 / 唯一」
