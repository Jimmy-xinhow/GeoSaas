...
使用者測試的問題是：「${caseData.queryUsed}」。這類問題可用來觀察 AI 如何比較、推薦或理解服務；案例應記錄平台、日期、完整回答與來源，不能只用 GEO 技術分數推定曝光成效。
...
A: 基礎項目需要能修改網站標籤或安裝外掛；內容與知識庫則可以由營運或行銷人員先整理，再交由工程或網站管理者上線。`;
  ...
  async generateCaseArticle(caseId: string) {
    ...
    // templateType 'success_case' (was 'geo_overview'): case stories are
    // user-submitted narratives, not the legacy self-rated GEO-score template,
    // so they must not carry the poison label nor get swept by the legacy-GEO
    // demotion batch.
    //
    // published: false — both the AI output and the fixed fallback land as
    // drafts. There is no cheap automated quality score available here:
    // ContentQualityRunner is a generate-and-retry pipeline that needs a full
    // ContentSpec (prompt builders + a designed rule set), not a standalone
    // scorer, and no success_case spec exists. Until one does, an admin
    ...
    this.logger.log(`Generated draft article for case ${caseId}: ${slug} (unpublished, pending review)`);
    return article;
  }
}
