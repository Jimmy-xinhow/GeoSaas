      ...
      <div className="bg-white/5 rounded-2xl border border-white/10 p-6">
        ...
        <div className="space-y-3">
          {[
            ...
            { rule: '在地服務要測地區意圖', detail: '在地服務可加入城市與區域，並保留不帶地區的對照題，才能判斷不同問法下的實際提及差異。' },
            { rule: '知識庫要先於問題集', detail: '知識庫用來整理可公開核對的品牌事實，問題集用來量測 AI 回答。先完成資料，再用固定題目驗收。' },
            { rule: '不要只測「品牌名+好不好」', detail: '這種問題 AI 本來就會搜到，不代表自然搜尋時會推薦你。要測的是消費者不知道你的情況下，AI 會不會主動推薦。' },
            { rule: '不同產業要分開建立基準', detail: '產業競爭度、資料來源與問題意圖不同，不能用另一個產業的結果推定推薦難度。' },
            { rule: '引用率低要拆解原因', detail: '先看未提及題型、來源與平台差異，再補足能由第一方資料支持的內容，使用同一問題集重測；改善幅度與輪次必須以實際結果為準。' },
          ].map((item) => (
