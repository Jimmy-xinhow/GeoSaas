...
function markdownToHtml(md: string): string {
  return md
    // Code blocks
    .replace(/```(\w*)\n([\s\S]*?)```/g, '<pre class="bg-gray-900 text-gray-100 rounded-lg p-4 overflow-x-auto my-4"><code class="language-$1">$2</code></pre>')
    // Inline code
    ...
    .replace(/^### (.+)$/gm, (_, text) => {
      ...
      return `<h3 id="${id}" class="text-lg font-bold text-gray-900 mt-8 mb-3">${text}</h3>`;
    })
    .replace(/^## (.+)$/gm, (_, text) => {
      const id = text.trim().toLowerCase().replace(/[^a-z0-9\u4e00-\u9fff]+/g, '-');
      return `<h2 id="${id}" class="text-xl font-bold text-gray-900 mt-10 mb-4">${text}</h2>`;
    })
    // Bold
    .replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>')
    // Ordered lists
    .replace(/^(\d+)\.\s+(.+)$/gm, '<li class="ml-6 list-decimal text-gray-700">$2</li>')
    // Unordered lists
    .replace(/^-\s+(.+)$/gm, '<li class="ml-6 list-disc text-gray-700">$1</li>')
    // Blockquotes
    .replace(/^>\s+(.+)$/gm, '<blockquote class="border-l-4 border-blue-500 pl-4 py-2 my-4 text-gray-600 italic">$1</blockquote>')
    // Horizontal rule
  ...
  if (!post) {
    return (
      <div className="text-center py-20">
        ...
        <Link href="/blog" className="text-blue-600 mt-4 inline-block">返回文章列表</Link>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <article className="max-w-3xl mx-auto px-6 py-8">
        {/* Back link */}
        <Link href="/blog" className="inline-flex items-center gap-1 text-sm text-gray-500 hover:text-gray-900 mb-6">
        ...
        {/* Table of Contents */}
        {headings.length > 3 && (
          <nav className="bg-gray-50 rounded-xl p-5 mb-8 border" aria-label="目錄">
        ...
        <div className="mt-12 p-8 bg-gradient-to-r from-blue-50 to-purple-50 rounded-2xl text-center">
          <h3 className="text-xl font-bold text-gray-900">想優化你的 AI 能見度？</h3>
          <p className="mt-2 text-gray-600">免費掃描你的網站，了解 AI 友善度分數</p>
