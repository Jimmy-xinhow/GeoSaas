...
5. AI 重新生成官網文章，通過事實規則、格式規則與三字元相似度閘門後保存為 `draft`。
6. 系統由 `publishBaseUrl + slug` 自動組出 canonical URL；舊版傳入完整 canonical URL 的客戶仍可相容。
7. 客戶審核後狀態變成 `export_ready`，才可取得 Markdown、CMS HTML、Meta 與 JSON-LD。
8. 客戶貼到自己的 CMS 後輸入正式 URL，驗證器檢查 HTML、canonical、Article JSON-LD、FAQ、OG 與 noindex。
...
| 網址 | `publishBaseUrl` 儲存客戶 CMS 集合位置，例如 `https://brand.com/blog`；canonical 由系統自動組合，slug 使用可讀的 ASCII 英數字，碰撞時才加 `-2`、`-3` |
